...
`watch-parse-markdown-scope` is centered on parsers work in Ruby. The technical target is: Implement a Ruby parsers project for markdown incremental indexing, using append-only fixtures and checkpoint recovery checks.
...
The score is deterministic. The current threshold is 160. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 2.
